...
int main()
{
	...
	try {
		...
		const GA::TextRendererClass::FontStruct& ArialFont = TEXT_RENDERER.AddFont(50, "Fonts/arial.ttf", U" !\"#$%&'()*+,-./0123456789:;<=>?@ABCDEFGHIJKLMNOPQRSTUVWXYZ[\\]^_"\
		...
		while (!window.WindowWaitingToClose()) {
			...
			TEXT_RENDERER.RenderText(ArialFont, L"english ðóññêèé ÉÆ¨!:(|&", Vector2F(-1.f,0.f), Vector2F(-1.f,0.f), Vector2U(Width, Height), Vector2F(1.f, 0.f), 0.5f);
